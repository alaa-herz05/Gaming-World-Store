...
// Get following count
$count_query = $conn->prepare("SELECT COUNT(*) AS total FROM follows WHERE follower_id = ?");
$count_query->bind_param("i", $current_user);
$count_query->execute();
$following_count = $count_query->get_result()->fetch_assoc()['total'];

// Get following (people I follow)
$following = $conn->prepare("
...
");
$following->bind_param("i", $current_user);
$following->execute();
$following_result = $following->get_result();
...
<head>
    <meta charset="UTF-8">
    <title>Following - Gaming World</title>
...
<header>
    ...
    <h1 class="site-logo"><i class="fa-solid fa-user-plus"></i> Following</h1>
...
<div class="container">
    ...
    <div class="stats-bar">
        ...
        <p><i class="fa-solid fa-user-plus"></i> People you follow</p>
    </div>

    <?php if($following_result && $following_result->num_rows > 0): ?>
        <?php while($follow = $following_result->fetch_assoc()): ?>
            <div class="user-card">
                <div class="user-info">
                    <?php if(!empty($follow['image'])): ?>
                        <img src="<?php echo htmlspecialchars($follow['image']); ?>" class="user-img">
                    <?php else: ?>
                    ...
                    <div class="user-details">
                        <h3 class="user-name">
                            <a href="user_profile.php?id=<?php echo $follow['id']; ?>">
                                <?php echo htmlspecialchars($follow['name']); ?>
                            </a>
                        </h3>
                        <p class="user-email"><?php echo htmlspecialchars($follow['email']); ?></p>
                    </div>
                </div>
                <div class="action-buttons">
                    <a href="messages_room.php?id=<?php echo $follow['id']; ?>" class="chat-btn">
                        <i class="fa-regular fa-message"></i> Chat
                    </a>
                    <form action="unfollow.php" method="POST" style="margin:0;">
                        <input type="hidden" name="user_id" value="<?php echo $follow['id']; ?>">
    ...
    <?php else: ?>
        <div class="no-results">
            ...
            <p style="margin-top: 10px; font-size: 12px;">
                <a href="users.php" style="color: #c94d06;">Find friends to follow!</a>
            </p>
        </div>
    <?php endif; ?>
</div>
